document.addEventListener('DOMContentLoaded', event => {
    ...
    safari.self.addEventListener(
      ...
      (msgEvent: any) => {
        ...
        if (msg.cmd === responseCommand && msg.data) {
          ...
          if (notificationBarData.neverDomains && notificationBarData.neverDomains.hasOwnProperty(window.location.hostname)) {
            return
          }
        ...
        processMessages(msg, () => {
          /* do nothing on send response for Safari */
        })
      },
      false
    )
  ...
  function processMessages(msg: any, sendResponse: Function) {
    if (msg.cmd === 'openNotificationBar') {
      if (inIframe) {
        return
      }
      ...
      sendResponse()
      return true
    } else if (msg.cmd === 'closeNotificationBar') {
      if (inIframe) {
        return
      }
      closeBar(true)
      sendResponse()
      return true
    } else if (msg.cmd === 'adjustNotificationBar') {
      if (inIframe) {
        return
      }
      adjustBar(msg.data)
      sendResponse()
      return true
    } else if (msg.cmd === 'notificationBarPageDetails') {
      ...
      observer = new MutationObserver(mutations => {
        ...
        for (let i = 0; i < mutations.length; i++) {
          ...
          for (let j = 0; j < mutation.addedNodes.length; j++) {
            ...
            if ((tagName != null && observeIgnoredElements.has(tagName)) || addedNode.querySelectorAll == null) {
              continue
            }
  ...
  function locatePassword(form: HTMLFormElement, passwordData: any, inputs: HTMLInputElement[], doLastFallback: boolean) {
  ...
  function formSubmitted(e: Event) {
    ...
    for (let i = 0; i < formData.length; i++) {
      ...
      if (!disabledChangedPasswordNotification && formData[i].passwordEls != null) {
        const passwords: string[] = formData[i].passwordEls.filter((el: HTMLInputElement) => el.value != null && el.value !== '').map((el: HTMLInputElement) => el.value)
  ...
  function getSubmitButton(wrappingEl: HTMLElement, buttonNames: Set<string>) {
    ...
    if (submitButton == null) {
      const possibleSubmitButtons = Array.from(wrappingEl.querySelectorAll('a, span, button[type="button"], ' + 'input[type="button"], button:not([type])')) as HTMLElement[]
  ...
  function closeExistingAndOpenBar(type: string, typeData: any) {
    ...
    switch (type) {
      case 'info':
        barPage = barPage + '?action=info&msg=' + typeData.text
        break
      case 'warning':
        barPage = barPage + '?action=warning&msg=' + typeData.text
        break
      case 'error':
        barPage = barPage + '?action=error&msg=' + typeData.text
        break
      case 'success':
        barPage = barPage + '?action=success&msg=' + typeData.text
        break
      case 'add':
        barPage = barPage + '?action=add'
        break
      case 'change':
        barPage = barPage + '?action=change'
        break
      default:
        break
    }
